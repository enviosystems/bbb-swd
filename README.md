...
```shell=
bbb_swd_silabs.patch
```

The BBG-SWD driver is ready for use.  It is tested with MGM210P MCU
...
The patch is: `bbb_swd_silabs.patch`
...
This patch will add support for SWD utlizing PRU module, as well as add support for
MGM210P MCU family (ARM Cortex M33) of Silicon Labs
...
Clone this repository in Beaglebone, together with all submodules
```shell=
git clone https://github.com/enviosystems/bbb-swd
cd bbb-swd
git submodule update --init --recursive
```
To install PRU driver simply run
```shell=
make -f am335x_pru_package/Makefile install
```
...
To build mgm-pru-swd.p run
```shell=
am335x_pru_package/pru_sw/utils/pasm -b -V3 mgm-pru-swd.p
```
I put the program for PRUSS in:
```shell=
sudo mv mgm-pru-swd.bin /usr/local/share/openocd/bbg-swd/pru-swd.bin
```
...
```   
IMPORTANT: You need to reset th MCU manually after flashing, as at the moment this is not handling the RESET pin

This is a binary for Zigbee NCP UART. To test it on UART1 run (assuming that Zigbee GW App is already installed)
```shell=
